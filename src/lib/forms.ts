...
/** Base URL of the B12 product, overridable via the `__PRODUCT_URL__` build-time constant. */
const PRODUCT_HOST = typeof __PRODUCT_URL__ === 'undefined' ? 'https://b12.io': __PRODUCT_URL__
/** Endpoint that all scaffold contact forms POST to. */
const CONTACT_FORM_SUBMISSION_URL = `${PRODUCT_HOST}/contact/send/`

/** Displays a submission result message, scrolling it into view. */
...
/** Wires up submission handling for a form: sets action URL, injects hidden fields (form ID, honeypot, next page, reCAPTCHA), and attaches the submit event listener. */
  ...
  formEl.addEventListener('submit', function(e) {
    ...
    function submitForm() {
      ...
      fetch(url, fetchOptions)
        ...
        .catch(function() {
          formErrorMessageElement.hidden = false
...
/** Finds all forms with a `data-form-id` attribute and the B12 action URL, and calls setupForm on each. Runs automatically on DOMContentLoaded. */
